...
[![PyPI](https://img.shields.io/pypi/v/cognis-entropyscan.svg)](https://pypi.org/project/cognis-entropyscan/)
[![CI](https://github.com/cognis-digital/entropyscan/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/entropyscan/actions)
...
ENTROPYSCAN exists for one job — flag packed/encrypted/high-entropy regions in files — and does it without a SaaS bill or heavyweight setup.
...
```bash
entropyscan --version
entropyscan scan .                      # scan the current project
entropyscan scan . --format json
entropyscan scan . --fail-on high       # non-zero exit for CI gates
entropyscan mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **binwalk entropy**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`entropyscan` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
