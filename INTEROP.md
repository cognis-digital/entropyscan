...
How **entropyscan** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `entropyscan` lives in the **Malware / binary** cluster.
...
| from | relation | to |
|---|---|---|
| `entropyscan` | JSON findings compose with | [`yaragen`](https://github.com/cognis-digital/yaragen), [`binhunt`](https://github.com/cognis-digital/binhunt) |
| `entropyscan` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `entropyscan` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `entropyscan` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the malware / binary cluster.** `entropyscan` output feeds the next tool:
```bash
entropyscan ... --format json > out.json          # this tool's findings (see `entropyscan --help`)
...
entropyscan ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
entropyscan ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
entropyscan ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `entropyscan`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `entropyscan` slots
