...
- **rules/analyzers** apply the heuristics shipped in `entropyscan/core.py`.
- **score** ranks by severity.
- **MCP server** (`entropyscan mcp`) exposes `scan` for Cognis.Studio agents.
