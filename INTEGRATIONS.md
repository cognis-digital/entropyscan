...
**entropyscan** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `entropyscan` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
entropyscan ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
entropyscan ... --format json | cognis-connect emit --to misp --url $URL --token $TOK   # MISP event
entropyscan ... --format json | cognis-connect emit --to sigma   # Sigma rules
```
...
works whatever `entropyscan` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="entropyscan") for rec in records]   # records = your JSON output
