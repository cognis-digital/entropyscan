...
<body>
  ...
  <div class="sub mono">{esc(report.path)} &mdash; {report.size:,} bytes &mdash; block {report.block_size} B {'(truncated)' if report.truncated else ''}</div>
  <div class="cards">
  ...
  <div class="sub">Generated by {esc(TOOL_NAME)} {esc(TOOL_VERSION)}. High Shannon entropy (&rarr;8.0 bits/byte) suggests compressed, encrypted, or packed data.</div>
</body></html>"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sub.add_parser(
        "mcp",
        help="Run the MCP stdio server (exposes scan() as an MCP tool). "
